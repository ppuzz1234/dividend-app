...
/* 온보딩·계좌 화면용 — "20년 뒤 매달 목표 생활비를 받으려면?" 역산 시나리오.
 * ① 목표 월 생활비 → 연 4% 인출 가정으로 필요한 은퇴 시점 기초자산(시드) 역산
 * ② 시드에서 현재 보유 금융자산(currentAssets)을 뺀 차액(gap)을 20년간 만들려면
 *    ETF_BENCHMARKS 각각의 연환산 수익률 기준으로 매달 얼마씩 불입해야 하는지
 *    역산 (배당 재투자 가정) */
...
export function projectRetirementScenario({ monthlyLivingCost, years = 20, tax = 0.154, currentAssets = 0 }) {
  const requiredNestEgg = (monthlyLivingCost * 12) / WITHDRAWAL_RATE;
  const gap = Math.max(0, requiredNestEgg - currentAssets); // 이미 모은 자산을 뺀 추가 필요액

  const perEtf = ETF_BENCHMARKS.map((etf) => {
    const requiredMonthly = requiredMonthlyFor(gap, {
      years,
  ...
  return { requiredNestEgg, gap, withdrawalRate: WITHDRAWAL_RATE, perEtf };
}
