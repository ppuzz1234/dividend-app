...
export function Result({ sim, allocation, chosen, years, reinvest, age, goalNestEgg, monthlyGoal, manualAccounts, income = 50_000_000, finIncome = 0, taxPref = "growth", isaRollover = "isa1", liquidity = null, perAccount = null, monthlyContribution = 0, existingAssets = 0, productAlloc = {}, cycle = "weekly", onNext }) {
  ...
  const taxSaving = useMemo(() => {
    const { rooms } = buildAccountRooms({ mydata: true, manual: manualAccounts, income, finIncome, age, monthlyContribution, taxPref, isaRollover, liquidity, perAccount });
    const rows = rooms
      .map((r) => {
    ...
    return { rows, total };
  }, [manualAccounts, income, finIncome, age, monthlyContribution, deductRate, taxPref, isaRollover, liquidity, perAccount]);
